...
$$
q_h(t)=
\frac{\exp(\kappa s_h(t))}
{\exp(\kappa s_h(t))+\exp(\kappa s_l(t))}
$$
...
$$
\Delta_{t+1}
=(1-\eta)\Delta_t+\rho\left[p_h(t)r_h(t)-p_l(t)r_l\right]
$$
...
$$
\Delta_{t+1}
=(1-\eta)\Delta_t+\rho\left[p_h(t)r_h-p_l(t)r_l\right]
$$
...
and $0\le\epsilon<1$. Then there exists at least one post-depletion step for
which:
...
interval of stale lock-in until the first time $\tau$ such that:
...
Since $\rho p_l(t)r_l\ge0$, we have:

$$
\Delta_{t+1}\le(1-\eta)\Delta_t
$$

By induction:

$$
\Delta_{T+s}\le\Delta_T(1-\eta)^s
$$

A sufficient condition for recovery is:

$$
\Delta_T(1-\eta)^s\le r_l
$$

Solving for $s$ gives:

$$
s\ge\frac{\log(r_l/\Delta_T)}{\log(1-\eta)}
$$

when:

$$
\Delta_T>r_l,\qquad 0<\eta<1
$$
...
T_{\mathrm{recover}}
\le
\left\lceil\frac{\log(r_l/\Delta_T)}{\log(1-\eta)}\right\rceil
$$
...
$$
\Delta_T>r_l,\qquad \rho=0,\qquad \eta=0
$$
...
$$
\Delta_{t+s}
=\Delta_T-\rho r_l\sum_{u=0}^{s-1}p_l(T+u)
$$
...
$$
s\ge\frac{2(\Delta_T-r_l)}{\rho r_l\epsilon}
$$
...
**Theorem.**  
Consider the two-route adaptive-memory process above with $0\le\epsilon<1$,
$0<\eta<1$, $\rho\ge0$, and $r_l>0$. Suppose the high route depletes at
time $T$, so $r_h(t)=0$ for $t\ge T$. If:
